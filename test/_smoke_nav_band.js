...
`)
  ...
  hexo.init().then(() => hexo.call('generate', {})).then(() => {
    ...
    if (name === 'single') {
      ...
      ok(html.includes('#page-header.nav-fixed #nav::before'), 'band layer missing')
      ok(/width:\s*100vw/.test(html), 'band layer not viewport-wide')
      ok(/height:\s*100vh/.test(html), 'band layer not viewport-tall')
      ok(/clip-path:\s*inset\(0 0 calc\(100vh - 60px\) 0\)/.test(html), 'band layer not clipped to the 60px bar')
      ...
      ok(!/background-attachment:\s*fixed/.test(bandBlock), 'band must not size cover against the 60px box')
